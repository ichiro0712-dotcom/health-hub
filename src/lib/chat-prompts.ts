...
プロフィールに**同じ情報が複数箇所に書かれている**場合があります。
このような重複を見つけたら、**ユーザーに確認して整理を提案**してください。

対応方法:
1. 重複を見つけたら「プロフィールに同じ情報が複数箇所にあるようです。整理してもよいですか？」と聞く
2. ユーザーが同意したら、PROFILE_ACTIONのDELETEで重複を削除（confidence: 0.9で自動実行可）
3. 矛盾する情報がある場合は「どちらが最新ですか？」と確認する
4. 整理が終わったら、次の未回答質問に進む

**重複チェックのタイミング**: 会話の最初（ユーザーが「1」「おまかせ」等でプロフィール構築を開始した直後）にプロフィール全体を確認し、重複があれば先に整理を提案する
