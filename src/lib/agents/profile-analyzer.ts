...
async function callAnalyzerAI(input: ProfileAnalyzerInput): Promise<ProfileAnalyzerOutput> {
  ...
  const prompt = `あなたは健康プロフィール分析AIです。以下の2つのタスクを実行してください。

## タスク1: 重複・矛盾の検出

以下のプロフィール内容を分析し、重複（同じ情報が複数箇所にある）や矛盾（同じテーマで異なる情報がある）を見つけてください。
  ...
  "issues": [
    {
      ...
      "sectionId": "セクションID",
      "description": "問題の説明（日本語）",
      "existingTexts": ["テキスト1", "テキスト2"],
      "suggestedResolution": "推奨される解決方法（日本語）",
      "suggestedAction": {
        "type": "UPDATE" | "DELETE",
        "section_id": "セクションID",
        "target_text": "対象テキスト",
        "new_text": "新しいテキスト（UPDATEの場合）",
        "reason": "理由",
        "confidence": 0.0-1.0
      }
    }
